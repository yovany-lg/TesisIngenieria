...
Generalmente, las pruebas de caja gris (\emph{Gray Box Testing}) son orientadas a pruebas de funcionalidad o dominios de intercambio (\emph{Business Domain}) \cite{Cap2Bib:SWTest, Cap2Bib:SWTesting}. Básicamente, las pruebas de funcionalidad son efectuadas para verificar la interacciones entre sistemas \cite{Cap2Bib:sommerville}.

La técnica de pruebas de caja negra (\emph{Black Box Testing}) se emplea (de manera superficial) en pruebas de funcionalidad sin observar la estructura interna \cite{Cap2Bib:SWTest, Cap2Bib:SWTesting}. Este tipo de pruebas están basados totalmente en los requerimientos y la especificación del sistema \cite{Cap2Bib:sommerville}.
...
\newpage
En la tabla~\ref{tab:cap4:TestCASpH1} se muestran las mediciones tomadas del dispositivo CAS del sensor de pH. Para esta prueba se hizo un barrido de voltaje en la entrada respetando el dominio definido en la sección~\ref{Cap3:DHWCASpH}. 
...
Las pruebas de comunicación consisten básicamente en verificar que la comunicación entre la RTU y la MTU se realice de manera satisfactoria, y que se lleve a cabo la comunicación entre el \emph{software} de la MTU y la base de datos. Este grupo de pruebas se ilustra en el diagrama de flujo de la figura~\ref{fig:cap4:GrayBoxTestComm}. 
...
\begin{figure}[htb]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Presentación final del transceptor RS-232/RS-485.} \label{fig:cap4:Pasarela232485}
\end{figure}
...
Para conectar el transceptor RS-232/RS-485 a la interfaz serie RS-232 se construye un cable serie que consta de un conector \emph{DB-9} hembra y un conector hembra de tres terminales\footnote{La configuración de terminales para el cable serie se describe en la sección~\ref{cap3:IHWRS232RS485}.} que se conecta a dicho transceptor (Figura~\ref{fig:cap4:Cableserie}). 

La fuente de alimentación utilizada para el transceptor RS-232/RS-485 es un convertidor de voltaje (o eliminador) de $4.5\volt$, el cual se muestra en la figura~\ref{fig:cap4:LabFuente5v}.
...
\begin{figure}[htb]
	\begin{center}
		\begin{tabular}{ccc}
			...
			\subfloat[Fuente de alimentación para el transceptor RS-232/RS-485.]{%
...
La configuración de conexiones para la comunicación entre la RTU y la MTU se muestra en la figura~\ref{fig:cap4:TestComm}. Como se observa, la conexión entre la RTU y el transceptor RS-232/RS-485 se realiza a través de un cable de par trenzado (UTP, \emph{Unshielded Twisted Pair}). 
...
\begin{figure}[htb]
	\begin{center}
		\begin{tabular}{cc}
			...
			\subfloat[Conexión del cabe UTP al transceptor y conexión a la \emph{notebook}.]{
